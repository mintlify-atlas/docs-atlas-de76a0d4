---
title: "Transactions"
description: "Start, commit, and roll back transactions to group SQL statements into a single atomic unit of work."
---

Transactions group multiple SQL statements into a single atomic unit of work. Either all statements succeed and are committed, or the entire transaction is rolled back and no changes are applied. Turso supports explicit transactions with `BEGIN`, `COMMIT`, and `ROLLBACK`, and also runs each standalone statement in an implicit transaction (autocommit mode).
...
Starts a new explicit transaction. While a transaction is open, all subsequent statements are part of that transaction until a `COMMIT` or `ROLLBACK` is issued.
...
Aborts the current transaction and reverts all changes made since `BEGIN`. The database is left in the state it was in before the transaction started.
...
When no explicit transaction is active, Turso wraps each individual statement in an implicit transaction. Each statement is automatically committed after it executes successfully, or automatically rolled back if it fails.
...
- [INSERT](/sql-reference/statements/insert) — insert rows
- [UPDATE](/sql-reference/statements/update) — update rows
- [DELETE](/sql-reference/statements/delete) — delete rows
