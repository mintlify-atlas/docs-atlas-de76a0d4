...
Removes rows from a table. Use a `WHERE` clause to target specific rows; omitting it deletes every row. To remove the table itself, use [`DROP TABLE`](/sql-reference/statements/drop-table).
...
- [DROP TABLE](/sql-reference/statements/drop-table) — remove an entire table
