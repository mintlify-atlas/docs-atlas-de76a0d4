...
Retrieves rows from one or more tables. `SELECT` is the primary way to read data in Turso and supports filtering, joining, aggregation, sorting, subqueries, CTEs, window functions, and set operations.
...
|-----------|-------------|
| `result_column` | Columns or expressions to return. Use `*` for all columns or `table.*` for all columns from a specific table. |
| `table_or_subquery` | Table name, aliased table, or parenthesized `SELECT`. |
| `DISTINCT` | Removes duplicate rows from the result set. |
| `expression` | Any valid SQL expression. |
| `ordering_term` | Expression followed by optional `ASC`/`DESC` and `NULLS FIRST`/`NULLS LAST`. |
| `compound_operator` | `UNION`, `UNION ALL`, `INTERSECT`, or `EXCEPT`. |
| `cte_name` | Name for a Common Table Expression defined in the `WITH` clause. |
| `window_name` | Name for a reusable window definition in the `WINDOW` clause. |
...
Specifies the source tables for the query. Accepts table names, aliased tables, subqueries, and join expressions.
...
<Note>
RIGHT JOIN is not supported. CROSS JOIN syntax is accepted but does not prevent join reordering as it does in SQLite.
</Note>
...
Returns all rows from both tables. When a row in either table has no match, the missing side's columns contain NULL.
...
Automatically joins on all columns with identical names in both tables.
...
Aggregate functions available with `GROUP BY`:
...
| `EXCEPT` | Returns rows from the first result set not in the second. |
...
`ORDER BY` applies to the entire combined result and must appear after the last `SELECT`.
...
- [UPDATE](/sql-reference/statements/update) — modify existing rows
...
- [EXPLAIN](/sql-reference/statements/explain) — inspect query execution plans
