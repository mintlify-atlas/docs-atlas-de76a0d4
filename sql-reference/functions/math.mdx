...
description: "Mathematical functions for trigonometry, logarithms, rounding, and arithmetic in Turso."
---
...
| `atan2(Y, X)` | Inverse tangent of Y/X, using signs of both arguments to determine quadrant | REAL |
...
| `trunc(X)` | Integer part of X, truncated toward zero | INTEGER |
...
| `log(B, X)` | Logarithm of X in base B. Both must be > 0, B ≠ 1 | REAL |
...
| Function | Description | Returns |
|----------|-------------|--------|
| `mod(X, Y)` | Remainder of X divided by Y. Returns NULL if Y is 0 | REAL |
...
- [Scalar Functions](/sql-reference/functions/scalar) — string, conditional, and type functions
- [Aggregate Functions](/sql-reference/functions/aggregate) — sum, avg, min, max
- [Expressions](/sql-reference/expressions) — arithmetic operators
